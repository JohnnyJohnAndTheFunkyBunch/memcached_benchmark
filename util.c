...
void writeBlock(int fd, void* buffer, int writeSize) {
#if DLEVEL == 1
  fprintf(stderr, "Going to write %d bytes\n", writeSize);
#endif
  ...
  for(i = 0; i < writeSize; i++){
    fprintf(stderr, "%8x ", *((unsigned char*)buffer+i));
    if(i % 4 == 3 && i != 0){
     fprintf(stderr, "\n");
    }
  }
...
#if DLEVEL == 1
  fprintf(stderr, "Done with write block\n");
#endif
...
#if DLEVEL == 1
  fprintf(stderr, "Going to read %d bytes\n", readSize);
#endif
...
#if DLEVEL == 1
  fprintf(stderr, "Done with read block\n");
#endif
